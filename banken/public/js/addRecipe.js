...
function initRecipeForm() {
    ...
    instructionCount =
        document.querySelectorAll(".instruction-step").length || 1;
    ...
    document
        ...
        ?.addEventListener("click", addInstructionStep);
...
function initSelects() {
    document.addEventListener("change", function (e) {
        ...
        if (e.target.id === "category-select" && e.target.value) {
            handleCategorySelect.call(e.target, e);
        }

        // Handle tag select
        if (e.target.id === "tag-select" && e.target.value) {
            handleTagSelect.call(e.target, e);
        }
    });
...
function handleCategorySelect(_e) {
    if (this.value) {
...
function updateRemoveButtons() {
    document
        .querySelectorAll(".remove-ingredient, .remove-instruction")
        .forEach((btn) => {
            btn.disabled = false;
        });
}
